// Calendar (week grid), Tickets list, Team, Reports, Settings.
    ...
    <div className="content">
      ...
      <div className="page">
        <div className="page-h">
          ...
          <div className="right">
            <div className="tabs">
              <div>Day</div><div className="on">Week</div><div>Month</div>
            </div>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--line)' }}>
            {days.map((d, i) => (
              <div key={d} style={{
                padding: '12px 14px', borderRight: i < 6 ? '1px solid var(--line)' : 'none',
                background: i === today ? 'var(--brand-soft)' : 'var(--surface-2)',
              }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', minHeight: 540 }}>
            {days.map((_, i) => (
              <div key={i} style={{
                ...
                padding: 8, background: i === today ? 'oklch(0.98 0.02 180)' : 'transparent',
                  ...
                  return (
                    <div key={k} className="card" style={{ padding: 8, fontSize: 11, cursor: 'pointer', borderColor: 'var(--line)' }}>
                      <div className="row gap-4">
                        <span className={'pill ' + e.tone} style={{ fontSize: 9, padding: '0 5px' }}>{['11:00', '10:00', '11:00', '12:00', '10:00', '11:00', '09:30'][i] || '—'}</span>
        ...
        {STAFF.map(s => (
          <div key={s.id} className="card" style={{ padding: 16 }}>
            <div className="row gap-12">
              ...
              <div className="grow">
                <div className="row gap-6"><b>{s.name}</b>{s.online && <span className="dot" style={{ width: 6, height: 6, borderRadius: 99, background: 'var(--good)' }}></span>}</div>
...
const ReportsPage = () => {
  ...
  const total = propRows.reduce((s, p) => ({
    revenue: s.revenue + p.revenue, cleaning: s.cleaning + p.cleaning,
    maintenance: s.maintenance + p.maintenance, turnovers: s.turnovers + p.turnovers,
  }), { revenue: 0, cleaning: 0, maintenance: 0, turnovers: 0 });
  ...
  <div className="content">
    ...
    <div className="page">
          ...
          <div className="row gap-4" style={{ background: 'var(--surface-2)', padding: 2, borderRadius: 8 }}>
            {[['7d', '7 days'], ['30d', 'April'], ['90d', 'Q2'], ['ytd', 'YTD']].map(([k, l]) => (
              <div key={k} onClick={() => setPeriod(k)}
                   style={{ padding: '4px 10px', fontSize: 12, fontWeight: 600, borderRadius: 6, cursor: 'pointer',
                            background: period === k ? 'var(--bg)' : 'transparent',
                            color: period === k ? 'var(--ink)' : 'var(--ink-2)',
                            boxShadow: period === k ? 'var(--shadow-1)' : 'none' }}>{l}</div>
            ))}
          </div>
      ...
      <div className="kpi-strip">
        <div className="kpi">
          <div className="l">Cost / turnover</div>
          <div className="v mono tnum">€{costPerTurnover}</div>
          <div className="delta up">−€4 vs prev</div>
        </div>
        <div className="kpi">
          <div className="l">On-time %</div>
          <div className="v mono tnum">94%</div>
          <div className="delta up">+2.1pp</div>
        </div>
        <div className="kpi">
          <div className="l">Avg ticket SLA</div>
          <div className="v mono tnum">2h 14m</div>
          <div className="delta up">−18m</div>
        </div>
        <div className="kpi">
          <div className="l">Revenue / cleaner-hr</div>
          <div className="v mono tnum">€{revPerHour}</div>
          <div className="delta up">+€6</div>
        </div>
        <div className="kpi">
          <div className="l">AI auto-replies</div>
          <div className="v mono tnum">218</div>
          <div className="delta">87% accepted</div>
        </div>
        <div className="kpi">
          <div className="l">Damage recovered</div>
          <div className="v mono tnum">€420</div>
          <div className="delta">3 charges</div>
        </div>
      </div>

      <div className="split" style={{ marginTop: 12 }}>
        <div className="card">
          <div className="card-h"><Icon name="chart" size={14} /><h3>Turnovers per day</h3>
            <div className="right"><span className="muted" style={{ fontSize: 11 }}>last 30 days</span></div>
          </div>
          <div style={{ padding: 16, height: 200, display: 'flex', alignItems: 'flex-end', gap: 3 }}>
            {[8,11,9,12,14,10,7,11,13,15,12,10,8,11,14,16,12,9,10,13,15,17,14,11,9,12,15,18,14,12].map((v, i) => (
              <div key={i} title={'Apr ' + (i + 1) + ': ' + v + ' turnovers'}
                   style={{ flex: 1, height: `${(v / 18) * 100}%`, background: i >= 27 ? 'var(--brand)' : 'var(--ink)', borderRadius: '3px 3px 0 0', opacity: i >= 27 ? 1 : 0.85 }}></div>
            ))}
          </div>
        ...
        <div className="card">
          ...
          <div style={{ padding: 14 }}>
            {[
              ['Plumbing',     24, '3h 12m', 'bad'],
              ['Electrical',   18, '1h 48m', 'warn'],
              ['Appliance',    16, '4h 22m', 'info'],
              ['Lock / access', 9, '38m',    'ghost'],
              ['Supplies',     12, '52m',    'good'],
              ['Damage',        4, '1h 04m', 'warn'],
              ['Other',         8, '2h 11m', 'ghost'],
            ].map(([l, n, sla, c]) => (
              <div key={l} className="row gap-8" style={{ padding: '8px 0', borderBottom: '1px dashed var(--line)' }}>
                <span style={{ width: 100, fontSize: 12 }}>{l}</span>
                <div className="grow" style={{ height: 8, background: 'var(--surface-2)', borderRadius: 99 }}>
                  <div style={{ width: `${(n / 24) * 100}%`, height: '100%', borderRadius: 99, background: `var(--${c === 'ghost' ? 'ink-3' : c})` }}></div>
                </div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="card-h"><Icon name="user" size={14} /><h3>Cleaner performance</h3>
          <div className="right"><span className="muted" style={{ fontSize: 11 }}>sorted by jobs</span></div>
        </div>
        <table className="tbl">
          <thead>
            <tr>
              ...
              <th style={{ width: 110 }}>Avg time</th>
              <th style={{ width: 130 }}>On-time</th>
              <th style={{ width: 110 }}>Guest score</th>
              <th style={{ width: 100 }}>Paid</th>
              <th style={{ width: 130 }}>€ / hour</th>
            </tr>
          </thead>
          <tbody>
                  ...
                  <td>
                    <div className="row gap-6">
                      <div style={{ flex: 1, height: 6, background: 'var(--surface-3)', borderRadius: 99, overflow: 'hidden' }}>
                        <div style={{ width: c.ontime + '%', height: '100%', background: c.ontime >= 95 ? 'var(--good)' : c.ontime >= 90 ? 'var(--brand)' : 'var(--warn)' }}></div>
                      </div>
                      <span className="mono tnum" style={{ fontSize: 11, width: 28 }}>{c.ontime}%</span>
                    </div>
                  </td>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="card-h"><Icon name="bldg" size={14} /><h3>Property economics</h3>
          <div className="right"><span className="muted" style={{ fontSize: 11 }}>April · gross figures</span></div>
        </div>
        <table className="tbl">
          <thead>
            <tr>
              <th>Property</th>
              <th style={{ width: 80 }}>Occ</th>
              <th style={{ width: 80 }}>Turnovers</th>
              <th style={{ width: 100 }}>Revenue</th>
              <th style={{ width: 110 }}>Cleaning</th>
              <th style={{ width: 110 }}>Maintenance</th>
              <th style={{ width: 110 }}>Cost / turnover</th>
          ...
          <tbody>
            {propRows.map(r => {
              ...
              return (
                <tr key={r.pid}>
                  ...
                  <td>
                    <span className={'pill ' + (margin >= 85 ? 'good' : margin >= 80 ? 'brand' : 'warn')}>{margin}%</span>
                  </td>
                </tr>
              );
            })}
            <tr style={{ borderTop: '2px solid var(--line)', background: 'var(--surface-2)', fontWeight: 700 }}>
              <td>Total</td>
              <td className="mono tnum">—</td>
              <td className="mono tnum">{total.turnovers}</td>
    ...
    <div className="content">
      ...
      <div className="page" style={{ maxWidth: 920 }}>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="card-h">
            <div className="row gap-8" style={{ padding: '4px 0' }}>
              ...
              <div>
                <h3 style={{ fontSize: 14 }}>Hostaway</h3>
                <div className="muted" style={{ fontSize: 11 }}>Property management system · pulls bookings, codes, guest counts</div>
              </div>
            </div>
          ...
          <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 12 }}>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Account</div>
              <div className="mono">acct_18hk_paris</div>
            </div>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Last sync</div>
              <div>2 minutes ago · 47 listings · 18 future bookings</div>
            </div>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Sync schedule</div>
              <select className="input select"><option>Every 5 minutes</option><option>Every 15 minutes</option><option>Hourly</option></select>
            </div>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Fallback PMS</div>
              <select className="input select"><option>None</option><option>Guesty</option><option>Smoobu</option><option>Booking.com (iCal)</option></select>
            </div>
          </div>
        ...
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="card-h">
            <div className="row gap-8" style={{ padding: '4px 0' }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--wa)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Icon name="msg" size={16} />
              </div>
              <div>
                <h3 style={{ fontSize: 14 }}>Green API · WhatsApp</h3>
                <div className="muted" style={{ fontSize: 11 }}>Listens to selected chats · AI parses to tickets</div>
              </div>
            </div>
          ...
          <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 12 }}>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Instance ID</div>
              <div className="mono">110258 1141</div>
            </div>
            <div>
              <div className="muted" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', fontWeight: 700, marginBottom: 4 }}>Phone</div>
              <div className="mono">+33 6 99 12 34 56</div>
            </div>
        ...
        {/* Hostaway field mapping */}
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="card-h"><Icon name="link" size={14} /><h3>Hostaway field mapping</h3>
            <div className="right"><span className="pill ghost">8 fields mapped</span></div>
          </div>
          <div style={{ padding: 0 }}>
            <table className="tbl" style={{ fontSize: 12 }}>
              <thead><tr><th>Hostaway field</th><th></th><th>HK Planner field</th><th>Transform</th></tr></thead>
              <tbody>
                {[
                  ['listing.name', 'property.name', '—'],
                  ['listing.address', 'property.address', '—'],
                  ['listing.timeZone', 'property.timezone', '—'],
                  ['listing.cleaningFee', 'property.defaultFee', 'currency'],
                  ['reservation.checkOutTime', 'turnover.out', 'time HH:mm'],
                  ['reservation.nextCheckIn', 'turnover.in', 'time HH:mm · null → "—"'],
                  ['reservation.numberOfGuests', 'turnover.guests', '—'],
                  ['reservation.nights', 'turnover.nights', '—'],
                  ['listing.customField "doorCode"', 'property.code', '—'],
                  ['guest.name', 'turnover.guestName', '—'],
                ].map(([h, k, x], i) => (
                  <tr key={i} style={{ cursor: 'default' }}>
                    <td className="mono" style={{ fontSize: 11 }}>{h}</td>
                    <td style={{ width: 24, textAlign: 'center', color: 'var(--ink-3)' }}><Icon name="arrR" size={12} /></td>
                    <td className="mono" style={{ fontSize: 11 }}>{k}</td>
                    <td className="muted" style={{ fontSize: 11 }}>{x}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="row gap-6" style={{ padding: 12, borderTop: '1px solid var(--line)' }}>
            <button className="btn sm"><Icon name="plus" size={11} />Add mapping</button>
            <button className="btn sm">Test on a booking</button>
          </div>
        </div>

        {/* Checklist templates */}
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="card-h"><Icon name="list" size={14} /><h3>Checklist templates</h3>
            <div className="right"><button className="btn sm primary"><Icon name="plus" size={11} />New template</button></div>
          </div>
          <div style={{ padding: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                { n: 'Standard turnover', items: 17, props: 5, default: true },
                { n: 'Studio quick-clean', items: 11, props: 1, default: false },
                { n: '3BR Family deep clean', items: 24, props: 1, default: false },
                { n: 'Deep clean (no checkin)', items: 22, props: 0, default: false },
              ].map(tpl => (
                <div key={tpl.n} className="card" style={{ padding: 12, borderColor: tpl.default ? 'var(--brand)' : undefined }}>
                  <div className="row"><b style={{ fontSize: 13 }}>{tpl.n}</b><span style={{ flex: 1 }}></span>{tpl.default && <span className="pill brand">default</span>}</div>
                  <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>{tpl.items} items · 5 groups</div>
                  <hr className="sep" />
                  <div className="muted" style={{ fontSize: 11, fontWeight: 600, marginBottom: 4 }}>Applied to {tpl.props} {tpl.props === 1 ? 'property' : 'properties'}</div>
                  <div className="row gap-4" style={{ flexWrap: 'wrap' }}>
                    {PROPS.slice(0, tpl.default ? 5 : tpl.props).map(p => <span key={p.id} className="pill ghost" style={{ fontSize: 10 }}>{p.name}</span>)}
                  </div>
                  <div className="row gap-6" style={{ marginTop: 10 }}>
                    <button className="btn sm">Edit items</button>
                    <button className="btn sm">Assign property</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* AI parser rules */}
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="card-h"><Icon name="sparkle" size={14} /><h3>WhatsApp parser rules</h3>
            <div className="right"><span className="pill brand">v3 · GPT-4o</span></div>
          </div>
          <div style={{ padding: 16 }}>
            <div className="muted" style={{ fontSize: 12, marginBottom: 12 }}>
              Each incoming message is matched against these rules in order. First match wins. AI fills the gaps when no rule matches.
            </div>
            <div className="card" style={{ background: 'var(--surface-2)', padding: 0 }}>
              <table className="tbl" style={{ fontSize: 11.5 }}>
                <thead><tr><th style={{ width: 40 }}>#</th><th>Trigger keywords</th><th>Type</th><th>Priority</th><th>Auto-create?</th><th></th></tr></thead>
                <tbody>
                  {[
                    [1, 'leak, leaking, water, flooding', 'plumbing', 'high', true],
                    [2, 'no hot water, cold shower, boiler', 'plumbing', 'urgent', true],
                    [3, 'stuck, locked out, key, handle', 'lock', 'urgent', true],
                    [4, 'wifi, internet, no signal', 'electrical', 'med', false],
                    [5, 'bulb, light out, dark', 'electrical', 'low', false],
                    [6, 'oven, fridge, hoover, vacuum, dishwasher', 'appliance', 'med', false],
                    [7, 'toilet paper, soap, supplies', 'general', 'low', true],
                  ].map(([n, kw, type, prio, auto]) => (
                    <tr key={n} style={{ cursor: 'default' }}>
                      <td className="mono muted">{n}</td>
                      <td className="mono" style={{ fontSize: 11 }}>{kw}</td>
                      <td style={{ textTransform: 'capitalize' }}>{type}</td>
                      <td><Prio p={prio} compact /></td>
                      <td>{auto ? <span className="pill good" style={{ fontSize: 10 }}>auto</span> : <span className="pill ghost" style={{ fontSize: 10 }}>review</span>}</td>
                      <td><Icon name="edit" size={12} style={{ color: 'var(--ink-3)', cursor: 'pointer' }} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="row gap-6" style={{ marginTop: 12 }}>
              <button className="btn sm"><Icon name="plus" size={11} />Add rule</button>
              <button className="btn sm">Test on history</button>
              <span style={{ flex: 1 }}></span>
              <span className="muted" style={{ fontSize: 11 }}>Rules cover 78% of last 200 messages · AI handles the rest</span>
            </div>
          </div>
        </div>
              ...
              <div className="row gap-12">
                ...
                <div className="row gap-8" style={{ width: 280 }}>
                  <input type="range" min="0.5" max="1" step="0.01" value={tweaks.aiConfidence}
                         onChange={e => setTweaks(t => ({ ...t, aiConfidence: parseFloat(e.target.value) }))}
                         style={{ flex: 1 }} />
...
const Toggle = ({ on, onChange }) => (
  <div onClick={() => onChange(!on)} style={{
    width: 38, height: 22, borderRadius: 99, background: on ? 'var(--brand)' : 'var(--surface-3)',
              ...
              {PROPS.map(p => {
                ...
                return (
                  <tr key={p.id} onClick={() => t && goto('property', { tid: t.id })}>
                    ...
                    <td>{t?.tickets > 0 ? <span className="pill warn">{t.tickets} open</span> : <span className="muted">—</span>}</td>
                    <td><Icon name="chevR" size={14} style={{ color: 'var(--ink-3)' }} /></td>
                  </tr>
                );
              })}
...
Object.assign(window, { TicketsListPage, CalendarPage, TeamPage, ReportsPage, SettingsPage, PropertiesPage });
