// Live Ops — real-time view of who's where, what just happened, what's late.

const LIVE_NOW = '14:32';

// Cleaner live state — on-site / en route / break / idle / late
const CLEANERS_LIVE = [
  {
    id: 'S1', name: 'Amina K.', avatar: 'AK',
    state: 'on-site',
    job: { tid: 'T1', prop: 'P1', label: 'Marais 2BR · Apt 3A', started: '13:42', eta: '15:00', progress: 72 },
    lastPing: '14:31', battery: 78, gps: 'rue de Bretagne · ±8m',
    next: { prop: 'P5', label: 'Marais 3BR · Apt 5B', at: '15:00', sameBuilding: true },
  },
  {
    id: 'S2', name: 'Jorge M.', avatar: 'JM',
    state: 'en-route',
    job: { tid: 'T2', prop: 'P2', label: 'Belleville Studio', started: null, eta: '14:50', progress: 0 },
    lastPing: '14:30', battery: 54, gps: 'metro Belleville · 6 min away',
    next: null,
  },
  {
    id: 'S3', name: 'Lin W.', avatar: 'LW',
    state: 'late',
    job: { tid: 'T3', prop: 'P3', label: 'Montmartre 1BR', started: null, eta: '13:30 → expected 14:50', progress: 0 },
    lastPing: '14:24', battery: 12, gps: 'metro Pigalle · 22 min away',
    next: null,
    flag: 'No-show on URGENT back-to-back · guest checks in 13:00',
  },
  {
    id: 'S4', name: 'Diane B.', avatar: 'DB',
    state: 'on-break',
    job: { tid: 'T6', prop: 'P6', label: 'Latin Studio · deep clean', started: '11:15', eta: '17:00', progress: 48 },
    lastPing: '14:32', battery: 91, gps: 'rue Mouffetard',
    next: null,
  },
  {
    id: 'S5', name: 'Rashid F.', avatar: 'RF',
    state: 'idle',
    job: null,
    lastPing: '11:45', battery: 100, gps: 'last seen Garage',
    next: { prop: 'P3', label: 'Montmartre · plumbing ticket', at: '15:30' },
  },
];

// Live activity feed — most recent first
const ACTIVITY = [
  { id: 'a1', time: '14:32', kind: 'ai',       icon: 'sparkle', who: 'Auto-reply',  text: 'Sent EN response to Tom R. · "Hot water back at 14:00"', meta: 'Confidence 92%', tone: 'info' },
  { id: 'a2', time: '14:31', kind: 'ping',     icon: 'pin',     who: 'Amina K.',    text: 'Living room photo · Marais Apt 3A', tone: 'good' },
  { id: 'a3', time: '14:28', kind: 'check',    icon: 'check',   who: 'Amina K.',    text: 'Checklist · 18/25 items done', tone: 'good' },
  { id: 'a4', time: '14:24', kind: 'late',     icon: 'bell',    who: 'Lin W.',      text: 'Detected 54min late at Montmartre — last ping at metro Pigalle', tone: 'bad', actions: ['Call', 'Reassign'] },
  { id: 'a5', time: '14:21', kind: 'ticket',   icon: 'wrench',  who: 'Jorge M.',    text: 'Reported wobbly chair → ticket #244 created', tone: 'warn' },
  { id: 'a6', time: '14:18', kind: 'guest',    icon: 'msg',     who: 'Tom R. · guest', text: '"No hot water + wifi down" · parsed urgent (78%)', tone: 'bad' },
  { id: 'a7', time: '14:15', kind: 'sync',     icon: 'refresh', who: 'Hostaway',    text: 'Pulled 3 new bookings · Apr 30, May 2, May 4', tone: 'ghost' },
  { id: 'a8', time: '14:08', kind: 'arrived',  icon: 'pin',     who: 'Jorge M.',    text: 'Left previous job · en route Belleville', tone: 'info' },
  { id: 'a9', time: '14:02', kind: 'photo',    icon: 'cam',     who: 'Diane B.',    text: 'On-break · lunch logged (15min)', tone: 'ghost' },
  { id: 'a10', time: '13:58', kind: 'check',   icon: 'check',   who: 'Diane B.',    text: 'Bedroom 2 done · photos uploaded', tone: 'good' },
  { id: 'a11', time: '13:51', kind: 'lock',    icon: 'check',   who: 'Amina K.',    text: 'Lockbox opened at Marais Apt 3A · code 4421', tone: 'good' },
  { id: 'a12', time: '13:42', kind: 'start',   icon: 'pin',     who: 'Amina K.',    text: 'Started job · Marais 2BR Apt 3A', tone: 'info' },
  { id: 'a13', time: '13:30', kind: 'guest-ok',icon: 'msg',     who: 'Sophie L. · guest', text: 'Replied "ok thank you" → thread closed', tone: 'good' },
  { id: 'a14', time: '13:18', kind: 'ai',      icon: 'sparkle', who: 'Auto-reply',  text: 'Held reply to Marc D. — confidence 0.62 below threshold', tone: 'warn' },
  { id: 'a15', time: '13:05', kind: 'restock', icon: 'box',     who: 'Diane B.',    text: 'Logged supplies use: 3× toilet roll, 1× hand soap', tone: 'ghost' },
];
...
const LiveOpsPage = () => {
  ...
  const [selectedCleaner, setSelectedCleaner] = React.useState(null);
        ...
        <div style={{ padding: '20px 24px 12px', borderBottom: '1px solid var(--line)' }}>
          <div className="row gap-12">
            ...
            <div className="row gap-6" style={{
              padding: '4px 10px', background: 'var(--good-soft)', border: '1px solid var(--good)',
              borderRadius: 99, fontSize: 11, fontWeight: 700, color: 'var(--good)' }}>
              <span style={{ width: 8, height: 8, borderRadius: 99, background: 'var(--good)', animation: 'pulse 1.6s infinite' }}></span>
              LIVE · {LIVE_NOW}
            </div>
        ...
        <div className="liveops-grid" style={{ flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--line)', overflow: 'auto' }}>
            ...
            {CLEANERS_LIVE.map(c => {
              ...
              return (
                <div key={c.id}
                     onClick={() => setSelectedCleaner(sel ? null : c.id)}
                     style={{
                       padding: '12px 14px', borderBottom: '1px solid var(--line)', cursor: 'pointer',
                       background: sel ? 'var(--brand-soft)' : c.state === 'late' ? 'var(--bad-soft)' : 'transparent',
                     }}>
                  <div className="row gap-10">
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      ...
                      <span style={{
                        ...
                        background: `var(--${sm.color === 'ghost' ? 'ink-3' : sm.color})`, border: '2px solid var(--bg)' }}></span>
                    </div>
                    <div className="grow" style={{ minWidth: 0 }}>
                      <div className="row gap-6"><b style={{ fontSize: 13 }}>{c.name}</b>
                  ...
                  {sel && (
                    <div style={{ marginTop: 10, padding: 10, background: 'var(--surface-2)', borderRadius: 8, fontSize: 11.5 }}>
                      <div className="row" style={{ marginBottom: 6 }}>
                        ...
                        <span className="mono tnum">{c.lastPing}</span>
                      </div>
                      <div className="row" style={{ marginBottom: 6 }}>
                        <span className="muted">Battery</span>
                        <span className="mono tnum" style={{ color: c.battery < 20 ? 'var(--bad)' : 'var(--ink)' }}>{c.battery}%</span>
                      </div>
                      <div className="row" style={{ marginBottom: 8 }}>
                        <span className="muted">GPS</span>
                        <span style={{ fontSize: 10.5 }}>{c.gps}</span>
                      </div>
                      {c.next && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div className="row gap-4" style={{ padding: '10px 14px', borderBottom: '1px solid var(--line)', background: 'var(--surface-2)' }}>
              ...
              {[['all', 'All'], ['alerts', 'Alerts'], ['guest', 'Guests'], ['ai', 'AI'], ['ping', 'Photos & pings']].map(([k, l]) => (
                <div key={k} onClick={() => setFeedFilter(k)} style={{
                  padding: '3px 8px', fontSize: 10.5, fontWeight: 600, borderRadius: 5, cursor: 'pointer',
                  background: feedFilter === k ? 'var(--ink)' : 'transparent',
                  color: feedFilter === k ? 'var(--bg)' : 'var(--ink-2)',
                }}>{l}</div>
              ))}
            </div>
            <div style={{ flex: 1, overflow: 'auto', padding: '8px 0', position: 'relative' }}>
              ...
              {filteredFeed.map((a, i) => (
                <div key={a.id} className="row gap-12" style={{
                  padding: '10px 14px', position: 'relative',
                  background: a.tone === 'bad' ? 'var(--bad-soft)' : 'transparent',
                  borderLeft: a.tone === 'bad' ? '3px solid var(--bad)' : '3px solid transparent',
                }}>
                  <span className="mono tnum" style={{ width: 38, fontSize: 11, color: 'var(--ink-3)', flexShrink: 0 }}>{a.time}</span>
                  <div style={{
                    width: 26, height: 26, borderRadius: 99, flexShrink: 0,
                    background: `var(--${a.tone === 'ghost' ? 'surface-3' : (a.tone || 'info') + '-soft'})`,
                    color: `var(--${a.tone === 'ghost' ? 'ink-3' : a.tone || 'info'})`,
          ...
          <div style={{ borderLeft: '1px solid var(--line)', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{
              ...
              background: 'linear-gradient(135deg, #f6f4ee 0%, #ece7d8 100%)',
              backgroundImage: 'radial-gradient(circle at 30% 40%, rgba(0,0,0,.04) 0, transparent 40%), radial-gradient(circle at 70% 60%, rgba(0,0,0,.05) 0, transparent 40%)',
              ...
              {[
                ...
                <div key={i} style={{ position: 'absolute', top: p.top, left: p.left, transform: 'translate(-50%,-50%)' }}>
                  <div style={{
                    ...
                    background: `var(--${p.state === 'ghost' ? 'ink-3' : p.state})`,
                  ...
                  {p.state === 'bad' && (
                    <div style={{ position: 'absolute', inset: -8, borderRadius: '50%', border: '2px solid var(--bad)', opacity: .4, animation: 'pulse 1.4s infinite' }}></div>
                  )}
                </div>
              ))}
              {/* property pins (subtle) */}
              {[{ top: 80, left: 180 }, { top: 100, left: 280 }, { top: 200, left: 100 }].map((p, i) => (
                <div key={'p' + i} style={{ position: 'absolute', top: p.top, left: p.left, width: 8, height: 8, borderRadius: 99, background: 'var(--ink)', opacity: .3 }}></div>
              ))}
              <div style={{ position: 'absolute', bottom: 8, left: 8, fontSize: 9, color: 'var(--ink-3)', background: 'rgba(255,255,255,.7)', padding: '2px 6px', borderRadius: 4 }}>
                Demo · Mapbox tiles in production
              </div>
            </div>
            ...
            <div style={{ padding: 12 }}>
              ...
              <div className="col gap-8">
                <div style={{ padding: 10, background: 'var(--bad-soft)', border: '1px solid var(--bad)', borderRadius: 8 }}>
                  <div className="row gap-6"><Icon name="bell" size={12} style={{ color: 'var(--bad)' }} />
                    <b style={{ fontSize: 12 }}>Lin W. · 54min late</b></div>
                ...
                <div style={{ padding: 10, background: 'var(--warn-soft)', border: '1px solid var(--warn)', borderRadius: 8 }}>
                  <div className="row gap-6"><Icon name="bell" size={12} style={{ color: 'var(--warn)' }} />
                    <b style={{ fontSize: 12 }}>Lin W. · battery 12%</b></div>
                  <div className="muted" style={{ fontSize: 11, marginTop: 4 }}>May lose tracking soon.</div>
                </div>
                <div style={{ padding: 10, background: 'var(--bad-soft)', border: '1px solid var(--bad)', borderRadius: 8 }}>
                  <div className="row gap-6"><Icon name="msg" size={12} style={{ color: 'var(--bad)' }} />
                    <b style={{ fontSize: 12 }}>Tom R. · "no hot water"</b></div>
...
const LiveStat = ({ label, value, color, icon, alert }) => (
  <div style={{
    padding: '12px 14px', borderRadius: 8, border: '1px solid var(--line)',
    background: alert ? 'var(--bad-soft)' : 'var(--bg)',
    position: 'relative',
  }}>
...
Object.assign(window, { LiveOpsPage });
