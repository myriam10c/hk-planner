// Owner portal — read-only view per property: revenue, costs, tickets, statement.

const OWNER_PROPS = {
  // owner identity → properties they own
...
// Synthetic month-to-date numbers per property (Apr 2026)
const OWNER_DATA = {
...
const Spark = ({ data, w = 120, h = 32, color = 'var(--brand)' }) => {
...
// Owner-side sidebar (simpler than manager)
const OwnerSidebar = ({ ownerId, setOwnerId }) => {
  ...
  const NAV_O = [
    ...
    { g: 'Properties' },
    ...owner.props.map(pid => ({ id: 'owner-prop:' + pid, label: propById(pid).name, ico: 'bldg', pid })),
  ];
  ...
  return (
    <div className={'sidebar' + (tweaks.sidebarCollapsed ? ' collapsed' : '')}>
      ...
      {NAV_O.map((it, i) =>
        ...
        <div key={it.id}
             className={'nav-item' + (route.page === (it.id.split(':')[0]) && (route.pid === it.pid || !it.pid) ? ' active' : '')}
             onClick={() => goto(it.id.split(':')[0], it.pid ? { pid: it.pid } : {})}>
...
const OwnerHome = ({ ownerId }) => {
  const { goto, role, setRole } = useStore();
  const owner = OWNER_PROPS[ownerId];
    ...
    <div className="content">
      ...
      <div className="page">
        ...
        <div className="kpi-strip">
          <div className="kpi">
            <div className="l">Net payout MTD</div>
            <div className="v mono tnum">€{total.payout.toLocaleString()}</div>
            <div className={'pill ' + (delta >= 0 ? 'good' : 'bad')} style={{ marginTop: 6 }}>{delta >= 0 ? '+' : ''}{delta}% vs March</div>
          </div>
          <div className="kpi">
            <div className="l">Revenue</div>
            <div className="v mono tnum">€{total.revenue.toLocaleString()}</div>
            <div className="muted" style={{ fontSize: 11, marginTop: 6 }}>{total.nights} booked nights</div>
          </div>
          <div className="kpi">
            <div className="l">Cleaning</div>
            <div className="v mono tnum">€{total.cleaning}</div>
            <div className="muted" style={{ fontSize: 11, marginTop: 6 }}>{Math.round(total.cleaning / props.length)} avg / property</div>
          </div>
          <div className="kpi">
            <div className="l">Maintenance</div>
            <div className="v mono tnum">€{total.maint}</div>
            <div className="muted" style={{ fontSize: 11, marginTop: 6 }}>{props.reduce((s, x) => s + x.d.ticketIds.length, 0)} tickets</div>
          </div>
        </div>

        <div className="card" style={{ marginTop: 12 }}>
          <div className="card-h"><Icon name="bldg" size={14} /><h3>Your properties</h3>
            <div className="right"><span className="muted" style={{ fontSize: 11 }}>Tap any property for full breakdown</span></div>
          </div>
          <table className="tbl">
            <thead>
              <tr>
                <th>Property</th>
                <th style={{ width: 120 }}>Occupancy</th>
                <th style={{ width: 110 }}>ADR</th>
                <th style={{ width: 130 }}>Trend</th>
                <th style={{ width: 110 }}>Revenue</th>
                <th style={{ width: 110 }}>Costs</th>
                <th style={{ width: 110 }}>Net payout</th>
                ...
                return (
                  <tr key={p.id} onClick={() => goto('owner-prop', { pid: p.id })}>
                    <td>
                      ...
                      <div className="muted" style={{ fontSize: 11 }}>{p.zone}</div>
                    </td>
                    <td>
                      <div className="row gap-6">
                        <div style={{ flex: 1, height: 6, background: 'var(--surface-3)', borderRadius: 99, overflow: 'hidden' }}>
                          <div style={{ width: d.occ + '%', height: '100%', background: 'var(--brand)' }}></div>
                        </div>
                        <span className="mono tnum" style={{ fontSize: 11, width: 28 }}>{d.occ}%</span>
                      </div>
                    </td>
                    ...
                    <td>
                      {d.ticketIds.length > 0
                        ? <span className="pill warn"><Icon name="wrench" size={11} />{d.ticketIds.length}</span>
...
const OwnerPropertyPage = ({ ownerId }) => {
  const { route, goto, tickets, role, setRole } = useStore();
  ...
  return (
    <div className="content">
      <Topbar crumbs={[
        { label: 'Dashboard', onClick: () => goto('owner-home') },
        { label: p.name },
      ]} right={
      ...
      <div className="page">
        ...
        <div className="split">
          <div className="col gap-12">
            <div className="card">
              <div className="card-h"><Icon name="cal" size={14} /><h3>Upcoming bookings</h3>
                <div className="right"><span className="muted" style={{ fontSize: 11 }}>Synced with Hostaway</span></div>
              </div>
              <table className="tbl">
          ...
          <div className="col gap-12">
            ...
            <div className="card">
              <div className="card-h"><Icon name="user" size={14} /><h3>Your team</h3></div>
              <div style={{ padding: 14 }}>
                <div className="row gap-8" style={{ marginBottom: 10 }}>
                  <Avi name="Manon Vidal" size="lg" />
                  <div className="grow">
                    <b style={{ fontSize: 13 }}>Manon Vidal</b>
                    <div className="muted" style={{ fontSize: 11 }}>Property manager · responds &lt; 2h</div>
                  </div>
                  <button className="btn sm"><Icon name="msg" size={11} />Chat</button>
                </div>
                <div className="muted" style={{ fontSize: 11 }}>Cleaners assigned: Amina K., Diane B.</div>
              </div>
            </div>
            <div className="card">
...
const OwnerStatementsPage = ({ ownerId }) => {
  const { goto, role, setRole } = useStore();
  const owner = OWNER_PROPS[ownerId];
  const months = ['April 2026', 'March 2026', 'February 2026', 'January 2026', 'December 2025', 'November 2025'];
  const props = owner.props.map(pid => ({ p: propById(pid), d: OWNER_DATA[pid] }));
  const totalThis = props.reduce((s, x) => s + x.d.payout, 0);

  return (
    <div className="content">
      <Topbar crumbs={[{ label: 'Statements' }]} right={
        <button className="btn ghost sm" onClick={() => setRole('manager')}>← Manager view</button>
      } />
      <div className="page">
        <div className="page-h">
          <div>
            <h1>Statements</h1>
            <div className="muted" style={{ fontSize: 13, marginTop: 2 }}>Monthly financial summaries · paid to FR76 ···· 4421</div>
          </div>
          <div className="right">
            <button className="btn"><Icon name="cam" size={14} />Tax report 2025</button>
          </div>
        </div>
        <div className="card">
          <table className="tbl">
            <thead>
              <tr><th>Month</th><th>Revenue</th><th>Costs</th><th>Mgmt fee</th><th>Net payout</th><th>Status</th><th style={{ width: 80 }}></th></tr>
            </thead>
            <tbody>
              {months.map((m, i) => {
...
// ─── Owner messages page ─────────────────────────────────────────
const OwnerMessagesPage = ({ ownerId }) => {
  const { goto, role, setRole } = useStore();
  const owner = OWNER_PROPS[ownerId];
  const thread = [
    { who: 'Manon Vidal', avatar: true, time: 'Yesterday 18:22', text: 'Hi Sophie — the shower at Marais is fixed. Plumber charged €60, will appear on April statement. All good for the May 2 check-in.' },
    { who: 'Sophie Lambert', mine: true, time: 'Yesterday 18:40', text: 'Thanks Manon! Any update on the deep clean at Bastille?' },
    { who: 'Manon Vidal', avatar: true, time: 'Yesterday 18:51', text: 'Scheduled for next Wednesday — Diane will do it after the morning turnover. I\'ll send photos.' },
    { who: 'Manon Vidal', avatar: true, time: 'Today 09:14', text: '📎 April statement preview · €5,381 net (up 11% vs March). Final invoice will go out May 1.' },
  ];

  return (
    <div className="content">
      <Topbar crumbs={[{ label: 'Messages' }]} right={
        <button className="btn ghost sm" onClick={() => setRole('manager')}>← Manager view</button>
      } />
      <div className="page" style={{ maxWidth: 720 }}>
        <div className="page-h">
          <div>
            <h1>Messages with Manon</h1>
            <div className="muted" style={{ fontSize: 13, marginTop: 2 }}>Property manager · usually replies within 2 hours</div>
          </div>
        </div>
...
const OwnerPortal = () => {
  ...
  const ownerId = 'O1'; // Sophie has the most properties to look at
...
Object.assign(window, { OwnerPortal, OwnerSidebar, OWNER_PROPS, OWNER_DATA });
