...
const OnboardingPage = () => {
  ...
  const [propMap, setPropMap] = React.useState(() => {
    const m = {}; PROPS.forEach(p => m[p.id] = { include: true, template: p.id === 'P5' ? 'Family' : 'Standard', cleaner: '' });
    return m;
  });
  ...
  const finish = () => {
    showToast('Setup complete · welcome to HK Planner');
    goto('today');
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'linear-gradient(140deg, var(--bg) 0%, var(--surface-2) 60%, var(--brand-soft) 100%)', display: 'flex', flexDirection: 'column', zIndex: 200 }}>
              ...
              <React.Fragment key={s.key}>
                <div className="col" style={{ alignItems: 'center', gap: 6, flex: '0 0 auto', minWidth: 80 }}>
                  <div style={{
                    ...
                    background: done ? 'var(--good)' : cur ? 'var(--ink)' : 'var(--bg)',
                    color: done || cur ? '#fff' : 'var(--ink-3)',
                    border: '2px solid ' + (done ? 'var(--good)' : cur ? 'var(--ink)' : 'var(--line)'),
                  }}>
                ...
                {i < ONBOARDING_STEPS.length - 1 && (
                  <div style={{ flex: 1, height: 2, background: i < step ? 'var(--good)' : 'var(--line)', marginTop: 14 }}></div>
                )}
              </React.Fragment>
            );
          })}
        ...
        <div style={{ width: '100%', maxWidth: 720 }}>
          ...
          {step === 0 && (
            <div className="col gap-12" style={{ textAlign: 'center', paddingTop: 40 }}>
              ...
              <p style={{ fontSize: 16, color: 'var(--ink-2)', maxWidth: 520, margin: '0 auto', lineHeight: 1.5 }}>
                We'll connect your channels, sync your properties, invite your team, and tune AI to match your style.
                Takes about 5 minutes — most of it is automatic.
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginTop: 24, textAlign: 'left' }}>
                {[
                  ['bldg', 'Hostaway sync', 'Auto-pulls properties + bookings'],
                  ['msg', 'WhatsApp inbox', 'AI parses messages → tickets'],
                  ['sparkle', 'Smart rules', 'Tune what auto-runs vs review'],
                ].map(([i, t, d]) => (
                  <div key={t} className="card" style={{ padding: 16 }}>
                    <Icon name={i} size={18} style={{ color: 'var(--brand)' }} />
          ...
          {step === 1 && (
            <div className="col gap-12">
              ...
              <p style={{ fontSize: 14, color: 'var(--ink-2)' }}>
                Paste your API key — we'll auto-import properties, bookings and turnover times. Read-only by default.
              </p>
              <div className="card" style={{ padding: 18 }}>
                ...
                <input className="input" value={hostawayKey} onChange={e => setHostawayKey(e.target.value)}
                       placeholder="ha_live_····················" style={{ width: '100%', fontFamily: 'var(--mono)', fontSize: 13 }} />
                <div className="muted" style={{ fontSize: 11, marginTop: 8 }}>
                  Find it in Hostaway: <b>Settings → API → Create new key</b> · Scope: read properties, read reservations.
                </div>
                <button className="btn primary" style={{ marginTop: 14 }}
                        disabled={!hostawayKey || discovered}
              ...
              <div className="card" style={{ padding: 14, background: 'var(--info-soft)' }}>
                <div className="row gap-8">
                  <Icon name="bell" size={14} style={{ color: 'var(--brand)', marginTop: 2 }} />
                  <div className="grow" style={{ fontSize: 12 }}>
                    <b>Don't have Hostaway?</b><br/>
                    <span className="muted">You can also start from CSV upload, or connect Guesty / Smoobu / Lodgify after setup.</span>
                  </div>
                  <button className="btn sm">Other PMS</button>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="col gap-12">
              ...
              <p style={{ fontSize: 14, color: 'var(--ink-2)' }}>
                Choose which to manage in HK Planner, pick a checklist template and (optional) a default cleaner.
              </p>
              <div className="card">
                <table className="tbl">
                    ...
                    {PROPS.map(p => (
                      <tr key={p.id}>
                        ...
                        <td>
                          ...
                          <div className="muted" style={{ fontSize: 11 }}>{p.zone} · synced from Hostaway</div>
                        </td>
                        <td>
                          <select className="input select" value={propMap[p.id].template} onChange={e => setPropMap({ ...propMap, [p.id]: { ...propMap[p.id], template: e.target.value } })}>
                            <option>Standard</option><option>Family</option><option>Studio</option><option>Custom…</option>
                          </select>
                        </td>
                        <td>
                          <select className="input select" value={propMap[p.id].cleaner} onChange={e => setPropMap({ ...propMap, [p.id]: { ...propMap[p.id], cleaner: e.target.value } })}>
          ...
          {step === 3 && (
            <div className="col gap-12">
              ...
              <p style={{ fontSize: 14, color: 'var(--ink-2)' }}>
                Guests + cleaners will message this number. We use Green API — no app switch, your existing WhatsApp Business stays the source of truth.
              </p>
              <div className="card" style={{ padding: 18 }}>
                ...
                <button className="btn primary" style={{ marginTop: 12 }} disabled={waPhone.length < 8 || verified} onClick={() => setTimeout(() => { setVerified(true); showToast('Number verified · webhook live'); }, 600)}>
                  ...
                  <div style={{ marginTop: 14, padding: 12, background: 'var(--good-soft)', borderRadius: 8 }}>
                    <div className="row gap-8">
                      ...
                      <div className="grow" style={{ fontSize: 12 }}>
                        ...
                        <div className="muted">Last test message received 12s ago · "ok thanks"</div>
                      </div>
                    </div>
                  </div>
                )}
          ...
          {step === 4 && (
            <div className="col gap-12">
              ...
              <p style={{ fontSize: 14, color: 'var(--ink-2)' }}>
                Cleaners and maintenance get a mobile link via WhatsApp — no login needed. They see only their own assignments.
              </p>
              <div className="card">
                <table className="tbl">
                  ...
                  <tbody>
                    {invites.map((inv, i) => (
                      <tr key={i}>
                        <td><input className="input" value={inv.name} onChange={e => { const n = [...invites]; n[i].name = e.target.value; setInvites(n); }} placeholder="Name" /></td>
                        <td><input className="input" value={inv.phone} onChange={e => { const n = [...invites]; n[i].phone = e.target.value; setInvites(n); }} placeholder="+33 6 ··" style={{ fontFamily: 'var(--mono)' }} /></td>
                        <td>
                          <select className="input select" value={inv.role} onChange={e => { const n = [...invites]; n[i].role = e.target.value; setInvites(n); }}>
                            <option>Cleaner</option><option>Cleaner / Lead</option><option>Maintenance</option>
                          </select>
                        </td>
                ...
                <div style={{ padding: 10, borderTop: '1px solid var(--line)' }}>
                  <button className="btn sm" onClick={() => setInvites([...invites, { name: '', phone: '', role: 'Cleaner' }])}>
                    <Icon name="plus" size={11} />Add another
                  </button>
                </div>
              </div>
              <div className="muted" style={{ fontSize: 11 }}>
                We'll WhatsApp each person a one-time link. They tap it to install the cleaner app.
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="col gap-12">
              ...
              <p style={{ fontSize: 14, color: 'var(--ink-2)' }}>
                You can change all of this later. Pick how aggressive the assistant should be on your behalf.
              </p>
              <div className="card" style={{ padding: 18 }}>
                <div className="row gap-12" style={{ marginBottom: 14 }}>
                  <div className="grow">
                    <b style={{ fontSize: 13 }}>Auto-reply on incoming WhatsApp</b>
                    <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>AI sends an acknowledgement when confidence ≥ threshold</div>
                  </div>
                  <SwitchRow on={rules.autoReply} onChange={v => setRules({ ...rules, autoReply: v })} />
                </div>
                <div className="row gap-12" style={{ marginBottom: 14 }}>
                  <div className="grow">
                    <b style={{ fontSize: 13 }}>Auto-create tickets</b>
                    <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>Skip review queue when confidence ≥ threshold (most teams turn this on after week 1)</div>
                  </div>
                ...
                <div style={{ marginTop: 8 }}>
                  <div className="row" style={{ marginBottom: 6 }}>
                    <b style={{ fontSize: 13 }}>Confidence threshold</b>
                    <span style={{ flex: 1 }}></span>
                  ...
                  <div className="muted" style={{ fontSize: 11, marginTop: 4 }}>
                    Higher = safer (more goes to review). Lower = faster (more auto-handled).
                  </div>
                </div>
                <hr className="sep" />
                <div className="row gap-12" style={{ marginTop: 8 }}>
                  <div className="grow">
                    <b style={{ fontSize: 13 }}>Quiet hours</b>
                    <div className="muted" style={{ fontSize: 11, marginTop: 2 }}>Hold non-urgent notifications during these hours</div>
                  </div>
      ...
      <div className="row gap-8" style={{ padding: '14px 32px', borderTop: '1px solid var(--line)', background: 'var(--bg)' }}>
        ...
        <span style={{ flex: 1 }}></span>
        <span className="muted" style={{ fontSize: 11 }}>Step {step + 1} of {ONBOARDING_STEPS.length}</span>
        {step < ONBOARDING_STEPS.length - 1
          ? <button className="btn primary" onClick={next}
                    disabled={(step === 1 && !discovered) || (step === 3 && !verified)}>
              Continue <Icon name="chevR" size={12} />
            </button>
...
const SwitchRow = ({ on, onChange }) => (
  <div onClick={() => onChange(!on)} style={{
    width: 44, height: 24, borderRadius: 99, background: on ? 'var(--brand)' : 'var(--surface-3)',
    position: 'relative', cursor: 'pointer', transition: 'background .15s', flexShrink: 0,
  }}>
    <div style={{
      position: 'absolute', top: 2, left: on ? 22 : 2, width: 20, height: 20, borderRadius: 99,
