// WhatsApp inbox (threaded V3 + triage queue) and Ticket detail.
...
const replyFor = (msg) => REPLY_TEMPLATES[msg.parsed.type] ? REPLY_TEMPLATES[msg.parsed.type](msg.who.split(' ')[0]) : `Thanks ${msg.who.split(' ')[0]} — looking into it now.`;

const AIReplyCard = ({ msg, threshold, onSend, onHold }) => {
  const conf = msg.parsed.confidence;
  const high = conf >= threshold;
  const [reply, setReply] = React.useState(replyFor(msg));
  const [editing, setEditing] = React.useState(false);
  const [countdown, setCountdown] = React.useState(high ? 8 : null);
          ...
          <div className="scroll-y" style={{ flex: 1, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {!linkedTicket && tweaks.autoReply && (
              <AIReplyCard msg={sel} threshold={tweaks.aiConfidence}
                onSend={(r) => showToast('AI reply sent · ' + sel.who)}
...
const TicketPage = () => {
  ...
  const stage = tk.stage || (tk.status === 'closed' ? 'closed' : tk.status === 'in-progress' ? 'accepted' : 'open');
            ...
            <div className="card">
              ...
              <div style={{ padding: 14 }}>
                ...
                <div className="row gap-6" style={{ fontSize: 12, padding: '6px 0' }}>
                  ...
                  <span className="mono tnum">€{tk.parts || 0}</span>
                </div>
                ...
                <div className="row gap-6" style={{ marginTop: 10 }}>
                  <select className="input select" defaultValue="owner" style={{ flex: 1, fontSize: 12 }}>
                    <option value="owner">Bill: Owner ({p.owner})</option>
...
Object.assign(window, { InboxPage, TicketPage });
