  ...
  try {
    ...
    if (action === "syncListings") {
      ...
      for (const l of listings) {
        ...
        const { error } = await sb.from("listing_config").upsert({ listing_id: lid, listing_name: l.name || "", bedrooms, price, unit_type: unitType, updated_at: new Date().toISOString() }, { onConflict: "listing_id" });
        if (!error) synced++;
      }
    ...
    if (action === "getAllData") {
      const [doneRes, assignRes, cleanerRes, templateRes, listingRes, timerRes, issueRes, recurRes, ticketRes, vendorRes, equipRes, prevRes, profileRes, cancelledRes, extraRes] = await Promise.all([
        ...
        sb.from("listing_config").select("listing_id, listing_name, bedrooms, price, custom_price, unit_type"),
        ...
        sb.from("maintenance_tickets").select("*").order("created_at", { ascending: false }).limit(200),
      ...
      (listingRes.data || []).forEach((r: any) => { listingPrices[r.listing_id] = { bedrooms: r.bedrooms, price: r.price, custom_price: r.custom_price, listing_name: r.listing_name, unit_type: r.unit_type }; });
