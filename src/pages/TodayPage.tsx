...
import { BUILDINGS, PROPS, STAFF, propById, staffById, turnoverUrgency } from '../store/data';
import { Icon, Status, Avi } from '../components/Icon';
import { Topbar } from '../components/Shell';

export const TodayPage: React.FC = () => {
  const { turnovers, goto, role, assignCleaners, showToast } = useStore();
  const [selected, setSelected] = React.useState<string[]>([]);
  const [assignOpen, setAssignOpen] = React.useState(false);
  const [groupBy, setGroupBy] = React.useState<'none' | 'building'>('building');
  ...
  const flatSorted = [...enriched].sort((a, b) => {
    const order: Record<string, number> = { urgent: 0, tight: 1, normal: 2 };
      ...
      <div className="page">
        ...
        {urgents.length > 0 && (
          <div style={{ marginTop: 12, border: '1px solid var(--bad)', borderRadius: 10, background: 'var(--bad-soft)', overflow: 'hidden' }}>
            <div className="row gap-10" style={{ padding: '10px 14px', background: 'var(--bad)', color: '#fff' }}>
              ...
              <b style={{ fontSize: 12.5 }}>{urgents.length} urgent · back-to-back</b>
            ...
            <div style={{ padding: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 8 }}>
              {urgents.map(t => {
                ...
                return (
                  <div key={t.id} onClick={() => goto('property', { tid: t.id })}
        ...
        <div className="toolbar">
          ...
          <div className="row gap-4" style={{ background: 'var(--surface-2)', padding: 2, borderRadius: 8 }}>
            {([['none', 'Flat'], ['building', 'By building']] as [string,string][]).map(([k, l]) => (
              <div key={k} onClick={() => setGroupBy(k as 'none' | 'building')}
                   style={{ padding: '4px 10px', fontSize: 11.5, fontWeight: 600, borderRadius: 6, cursor: 'pointer',
                            background: groupBy === k ? 'var(--bg)' : 'transparent', color: groupBy === k ? 'var(--ink)' : 'var(--ink-2)' }}>{l}</div>
            ))}
          </div>
        ...
        <div className="card">
          ...
          <table className="tbl">
            ...
            <tbody>
              {(groupBy === 'building'
                ? groupedByBuilding.flatMap(g => [{ __header: g.building, count: g.jobs.length } as any, ...g.jobs])
                : flatSorted
              ).map((row, idx) => {
                if (row.__header) {
                  const b = row.__header;
                  const grpJobs = groupedByBuilding.find(g => g.building.id === b.id)?.jobs || [];
                  const allCleaners = [...new Set(grpJobs.map((j: any) => j.cleaner).filter(Boolean))];
                  return (
                    <tr key={'h-' + b.id} style={{ background: 'var(--surface-2)' }}>
                      <td colSpan={9} style={{ padding: '8px 14px' }}>
                        <div className="row gap-10">
                          ...
                          {row.count > 1 && allCleaners.length === 1 && <span className="pill good" style={{ fontSize: 10 }}><Icon name="check" size={10} />Same cleaner</span>}
                          {row.count > 1 && allCleaners.length > 1 && <span className="pill warn" style={{ fontSize: 10 }}><Icon name="bell" size={10} />Different cleaners — consider batching</span>}
                          {row.count > 1 && allCleaners.length === 0 && <span className="pill bad" style={{ fontSize: 10 }}>No cleaner — assign one for all units</span>}
                        </div>
                      </td>
                    </tr>
                  );
                ...
                return (
                  <tr key={t.id} className={sel ? 'selected' : ''} onClick={() => goto('property', { tid: t.id })}>
                    <td><span className={'check' + (sel ? ' on' : '')} onClick={(e) => { e.stopPropagation(); toggleSel(t.id); }}></span></td>
                    <td>
                    ...
                    <td>
                      {s ? <div className="row gap-6"><Avi name={s.name} size="sm" />{s.name}</div>
...
const AssignModal: React.FC<{ turnovers: any[]; onClose: () => void; onAssign: (m: Record<string,string>) => void }> = ({ turnovers, onClose, onAssign }) => {
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20,18,14,.45)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} className="card pop" style={{ width: 560, maxWidth: '90vw', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        <div className="card-h">
          ...
          <div className="right"><button className="btn ghost sm" onClick={onClose}><Icon name="x" size={14} /></button></div>
        </div>
      ...
      <div className="mob-shell">
        <div className="mob-screen">
                ...
                <div style={{ padding: '0 18px 16px' }}>
                  ...
                  <div className="card pop" style={{ borderColor: 'var(--ink)', borderWidth: 2, padding: 16 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                      <div><div className="muted" style={{ fontSize: 10 }}>Out</div><div className="mono tnum" style={{ fontWeight: 700, fontSize: 14 }}>{current.out}</div></div>
                      <div><div className="muted" style={{ fontSize: 10 }}>Next in</div><div className="mono tnum" style={{ fontWeight: 700, fontSize: 14 }}>{current.in}</div></div>
                      <div><div className="muted" style={{ fontSize: 10 }}>Stay</div><div style={{ fontWeight: 700, fontSize: 14 }}>{current.guests}·{current.nights}n</div></div>
                    </div>
                    {current.notes && (
                  ...
                  return (
                    <React.Fragment key={t.id}>
                      <div className="row gap-12" style={{ alignItems: 'flex-start', marginBottom: 4, position: 'relative' }}>
                        <div style={{ width: 30, height: 30, borderRadius: 999, background: isDone ? 'var(--good)' : isCurrent ? 'var(--ink)' : 'var(--bg)', border: '2px solid ' + (isDone ? 'var(--good)' : isCurrent ? 'var(--ink)' : 'var(--line)'), color: isDone || isCurrent ? '#fff' : 'var(--ink-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0, zIndex: 1 }}>
                          {isDone ? <Icon name="check" size={13} /> : i + 1}
                        </div>
                        <div className="card grow" style={{ padding: 12, borderColor: isCurrent ? 'var(--ink)' : 'var(--line)', borderWidth: isCurrent ? 2 : 1 }} onClick={() => goto('property', { tid: t.id })}>
                          <div className="row">
          ...
          <div className="mob-tabs">
            {([['home', 'Today', true], ['msg', 'Tickets', false], ['cal', 'Schedule', false], ['user', 'Me', false]] as [string,string,boolean][]).map(([ic, l, on]) => (
