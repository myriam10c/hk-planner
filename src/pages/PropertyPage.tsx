...
import { propById, staffById } from '../store/data';
import { Icon, Status, Prio } from '../components/Icon';
...
const DamageModal: React.FC<{ property: any; turnoverId: string; onClose: () => void }> = ({ property, onClose }) => {
...
const CleanerProperty: React.FC<{ t: any; p: any; pct: number; done: number; total: number }> = ({ t, p, pct, done, total }) => {
      ...
      <div className="mob-shell">
        <div className="mob-screen">
            ...
            {checklist.map((g, gi) => (
              <div key={g.g}>
                <div style={{ padding: '10px 18px 6px', background: 'var(--surface-2)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--ink-3)', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{g.g}</span>
                  <span className="mono">{g.items.filter((i: any) => i.done).length}/{g.items.length}</span>
                </div>
          ...
          <div className="mob-tabs">
            {[['home', 'Today', true], ['msg', 'Tickets', false], ['cal', 'Schedule', false], ['user', 'Me', false]].map(([i, l, on]) => (
              <div key={String(l)} className={'mob-tab' + (on ? ' active' : '')}>
                <Icon name={String(i)} size={20} />
                <span>{l}</span>
              </div>
            ))}
          </div>
...
export const PropertyPage: React.FC = () => {
  ...
  const done = checklist.reduce((n, g) => n + g.items.filter((i: any) => i.done).length, 0);
              ...
              {checklist.map((g, gi) => (
                <div key={g.g} style={{ marginTop: gi === 0 ? 0 : 14 }}>
                  <div className="row" style={{ marginBottom: 6 }}>
                    ...
                    <span style={{ flex: 1 }}></span>
                    <span className="muted mono" style={{ fontSize: 10 }}>{g.items.filter((i: any) => i.done).length}/{g.items.length}</span>
                  </div>
